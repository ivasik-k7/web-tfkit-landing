...
const themes = ['Dark', 'Light', 'Cyber', 'GitHub Dark', 'Monokai', 'Solarized Light', 'Dracula', 'Atom One Dark', 'Gruvbox Dark', 'Night Owl'];
export function VisualizationShowcase() {
  const [activeView, setActiveView] = useState('graph');
  const activeVisualization = visualizations.find(v => v.id === activeView);
  return <div className="w-full py-24 px-6 bg-black">
      <div className="max-w-7xl mx-auto">
        ...
        once: true
          ...
          {visualizations.map(viz => <button key={viz.id} onClick={() => setActiveView(viz.id)} className={`flex items-center gap-3 px-6 py-4 rounded-xl border transition-all duration-300 ${activeView === viz.id ? 'border-cyan-500 bg-cyan-500/10 text-cyan-400' : 'border-cyan-500/20 bg-gray-900/50 text-gray-400 hover:border-cyan-500/40 hover:bg-gray-900/70'}`}>
        ...
        duration: 0.5
          ...
          {activeView === 'graph' ? <GraphDemo /> : activeView === 'dashboard' ? <DashboardDemo /> : <ClassicLayoutDemo />}
          {/* Description */}
          <motion.div initial={{
          opacity: 0,
          y: 10
        ...
        once: true
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {themes.map(theme => <span key={theme} className="px-4 py-2 rounded-full border border-cyan-500/30 bg-cyan-500/5 text-cyan-400 text-sm font-medium hover:bg-cyan-500/10 hover:border-cyan-500/50 transition-all">
                {theme}
              </span>)}
          </div>
        </motion.div>
